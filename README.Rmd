...
- 𝄜 Import a `data.frame` within the R programming environment
   - 📅 Types are inferred according to Excel `.xlsx` conventions, e.g., `Date`, `character`, and `numeric` values^[Note that R `data.frame` handle a single type of value within a column, i.e. a vector with several types is turned into a `character` vector]
